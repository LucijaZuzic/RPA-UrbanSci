...
for month_use in range(1, 13):
    metric_values = pd.read_csv("month_result/" + str(month_use) + "/metrics_" + str(month_use) + ".csv", index_col = False)
...
for month_range in [range(1, 7), range(7, 13)]:
    ...
    start_table += "\\caption{RPA results for hourly median horizontal errors (\\textdegree) in months from "
